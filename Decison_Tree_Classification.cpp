...
class Decision_Tree {
    ...
    int Max_depth;
    Node* Tree; 
    string spliting_function = "Gini";
public:
    Decision_Tree(vector<vector<string>> X_train, Eigen::VectorXd y_train, int Max_depth = -1, string Splitting_function = "Information_Gain", int Min_depth = -1, string spliting_function = "Gini") { // ! Add comma and correct the Gini function definition
        this->X_train = X_train;
        this->y_train = y_train;
        combine_data();
        this->Max_depth = Max_depth;
        this->spliting_function = spliting_function;
...
private:
    Node* Fit_Private(Node* currentNode, int depth) { 
        if (is_pure(*currentNode)) { 
            ...
            currentNode->class_ = to_string(y_train[currentNode->split.first]); // ! y_train is Eigen::VectorXd, should convert to string
            return currentNode; 
        }
...
private:
    double split_gain(const vector<vector<string>>& X_train, const pair<int, int>& left_threshold, const pair<int, int>& right_threshold) {
        if (spliting_function == "Gini") {
            return Gini(X_train, left_threshold, right_threshold);
        } 
        if (spliting_function == "information_gain") { // ! Comparison with string should use double quotes
    ...
    double Information_Gain(const vector<vector<string>>& data, const pair<int, int>& left_threshold, const pair<int, int>& right_threshold) {
        double H_S = calculate_entropy(pair<int, int>(0, y_train.size())); // ! Pass a pair to calculate_entropy
    ...
    double Gini(const vector<vector<string>>& data, const pair<int, int>& left_threshold, const pair<int, int>& right_threshold) {
        ...
        for (auto& count : left_class_counts) {
            left_gini += pow(static_cast<double>(count.second) / (left_threshold.second - left_threshold.first), 2); // ! Use static_cast<double> for division
        }
        for (auto& count : right_class_counts) {
            right_gini += pow(static_cast<double>(count.second) / (right_threshold.second - right_threshold.first), 2); // ! Use static_cast<double> for division
        }
...
private:
     string predict_recursive(const vector<string>& data, Node* currentNode) { // ! Return type should be string
